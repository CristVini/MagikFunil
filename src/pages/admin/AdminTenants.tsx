...
import { Loader2, Building2, Users, Shield, Crown, MoreVertical, Plus, Search, Edit, Trash2, Activity, X, Check } from 'lucide-react';
...
export function AdminTenants() {
  ...
  const updateStatus = async (tenantId: string, newStatus: TenantStatus) => {
    try {
  ...
  return (
    <div className="space-y-6" style={{ fontFamily: 'var(--font-sans)' }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { status: 'active' as TenantStatus, label: 'Ativos', color: 'bg-green-500/10 text-green-600', icon: Activity },
          { status: 'paused' as TenantStatus, label: 'Pausados', color: 'bg-amber-500/10 text-amber-600', icon: Users },
          { status: 'blocked_billing' as TenantStatus, label: 'Bloqueados', color: 'bg-red-500/10 text-red-600', icon: Shield },
          ...
          <div key={i} className="bg-white rounded-xl border border-stone-200 p-4 text-center">
            <p className="text-2xl font-bold text-stone-950">
              {stat.label === 'Total' ? tenants.length : tenants.filter(t => t.status === stat.status).length}
            </p>
                  ...
                  .map(tenant => (
                    <tr key={tenant.id} className="hover:bg-stone-50">
                      ...
                      <td className="px-6 py-4">
                        <select
                          value={tenant.status}
                          onChange={e => updateStatus(tenant.id, e.target.value as TenantStatus)}
                          className={cn(
                            'px-3 py-1.5 rounded-full text-xs font-semibold border',
                            tenant.status === 'active' ? 'bg-green-500/10 text-green-600 border-transparent' :
                            tenant.status === 'paused' ? 'bg-amber-500/10 text-amber-600 border-transparent' :
                      ...
                      <td className="px-6 py-4 text-sm text-stone-500">{tenant.template_id || '—'}</td>
...
const Activity = Building2; // fallback