...
const STATUS_LABEL: Record<string, string> = { active: "Ativa", trial: "Trial", unpaid: "Em atraso", canceled: "Cancelada" };
...
const EVENT_LABEL: Record<string, string> = {
  trial_start: "Início de trial", payment: "Pagamento", invoice_paid: "Fatura paga",
  invoice_failed: "Fatura recusada", plan_change: "Mudança de plano", refund: "Reembolso",
};
const EVENT_STYLE: Record<string, string> = {
  trial_start: "bg-purple-500/10 text-purple-600", payment: "bg-green-500/10 text-green-600",
    ...
    <div className="fixed inset-0 z-50 bg-stone-950/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-lg w-full p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-stone-950">{initial ? "Editar Plano" : "Novo Plano"}</h2>
          <button onClick={onClose} className="p-2 text-stone-400 hover:text-stone-600"><span className="text-xl">×</span></button>
        </div>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1">Nome</label>
            <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Ex: Growth"
          ...
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1">Preço mensal (R$)</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Máx. leads/mês</label>
              <input type="number" value={form.max_leads_per_month} onChange={e => setForm({ ...form, max_leads_per_month: parseInt(e.target.value || "0") })}
                className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-stone-950 focus:outline-none focus:ring-2 focus:ring-amber-500" />
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <label className="flex items-end gap-2 pb-3 cursor-pointer">
              <input type="checkbox" checked={form.custom_domain} onChange={e => setForm({ ...form, custom_domain: e.target.checked })}
    ...
    <div className="space-y-6" style={{ fontFamily: 'var(--font-sans)' }}>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-stone-500 mt-1">Monetização e status financeiro dos tenants</p>
        </div>
        {tab === "planos" && (
      ...
      <div className="flex gap-2 border-b border-stone-200">
        {[
          ...
          { key: "assinaturas" as const, label: "Assinaturas", icon: CreditCard },
          { key: "billing" as const, label: "Eventos de billing", icon: HandCoins },
        ].map(t => (
      ...
      {tab === "planos" && (
        <div className="grid md:grid-cols-3 gap-5">
          {plans.map(p => (
            <div key={p.id} className="bg-white rounded-2xl border border-stone-200 p-6 flex flex-col">
              <div className="flex items-start justify-between mb-3">
                <div className="p-3 rounded-xl bg-amber-500/10 text-amber-600"><Crown size={22} /></div>
                <div className="flex gap-1">
                  <button onClick={() => { setEditing(p); setShowModal(true); }} className="p-2 text-stone-400 hover:text-amber-500 rounded-lg transition-colors"><Edit size={16} /></button>
                  <button onClick={() => handleDelete(p.id)} className="p-2 text-stone-400 hover:text-red-500 rounded-lg transition-colors"><Trash2 size={16} /></button>
                </div>
              </div>
              <h3 className="text-lg font-bold text-stone-950">{p.name}</h3>
              <p className="text-3xl font-bold text-stone-950 mt-2">{formatCurrency(p.price_monthly_cents / 100)}<span className="text-base font-medium text-stone-400">/mês</span></p>
              <p className="text-xs text-stone-500 mt-1">{p.trial_days} dias de trial • sem plano gratuito</p>
              <ul className="mt-4 space-y-2 flex-1">
                {[
                  `Até ${p.max_products} produtos`,
                  `${p.max_leads_per_month.toLocaleString('pt-BR')} leads/mês`,
                  p.custom_domain ? "Domínio próprio" : "Subdomínio .seudominio.com",
                ].map((f, i) => <li key={i} className="flex items-center gap-2 text-sm text-stone-700"><Check size={15} className="text-green-500 shrink-0" /> {f}</li>)}
              </ul>
            </div>
          ))}
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="bg-stone-50 border-b border-stone-200">
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold text-stone-500 uppercase tracking-wider">Provider</th>
      ...
      {tab === "billing" && (
        <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
          <div className="p-5 border-b border-stone-200 flex items-center justify-between">
            <h3 className="font-semibold text-stone-950 flex items-center gap-2"><HandCoins size={18} className="text-amber-500" /> Histórico de cobrança</h3>
            ...
            {billingEvents.map(e => (
              <div key={e.id} className="px-6 py-4 flex items-center gap-4 hover:bg-stone-50">
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-medium text-stone-950">{e.amount_cents > 0 ? formatCurrency(e.amount_cents / 100) : '—'}</p>
                  <p className="text-xs text-stone-400">{new Date(e.created_at).toLocaleDateString('pt-BR')}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {showModal && <PlanModal initial={editing} onClose={() => { setShowModal(false); setEditing(null); }} onSave={handleSave} />}
    </div>
  );
}