...
export function Onboarding() {
  ...
  const finish = async () => {
    setLoading(true);
    try {
      // Garante que há sessão válida (o create_tenant_with_trial exige auth.uid())
      const {
        data: { session },
      } = await supabase.auth.getSession();
      if (!session) {
        setError(
          "Você precisa entrar (e confirmar seu e-mail) antes de criar seu funil.",
        );
        setLoading(false);
        navigate("/login");
        return;
      ...
      const tenantId = await supabase.rpc("create_tenant_with_trial", {
        p_slug: slug,
        p_name: brandName,
      ...
      setError(
        "Não foi possível criar seu funil. Verifique se o endereço já não está em uso e tente novamente.",
      );
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-2xl">
        <div className="text-center mb-10">
          ...
          <div className="flex items-center justify-center gap-2 mb-8">
            {[1, 2].map((i) => (
              <div key={i} className="flex items-center gap-2">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${step >= i ? "bg-amber-500 text-stone-950" : "bg-stone-200 text-stone-500"}`}
                >
                  {step > i ? <Check size={16} /> : i}
                </div>
                {i < 2 && (
                  <div
                    className={`w-12 h-0.5 rounded ${step > i ? "bg-amber-500" : "bg-stone-200"}`}
                  />
                )}
              </div>
            ))}
          ...
          <p className="text-stone-500 mt-2">
            Configure seu funil em 2 passos rápidos
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-8 shadow-sm">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-2 flex items-center gap-2">
                  <Building2 size={16} className="text-amber-500" /> Nome da sua
                  marca *
                </label>
                <input
                  type="text"
                ...
                <p className="text-xs text-stone-500 mt-1">
                  Como seus clientes verão sua marca no funil
                </p>
              </div>

              <div>
                <label className="block text-sm font-medium text-stone-700 mb-2 flex items-center gap-2">
                  <Globe size={16} className="text-amber-500" /> Endereço do seu
                  funil
                </label>
              ...
              <button
                onClick={() => {
                  if (!brandName.trim()) {
                    setError("Informe o nome da sua marca");
                    return;
                  }
                  if (!slug.trim()) {
                    setError("Informe o endereço do seu funil");
                    return;
                  }
                  setError("");
                  setStep(2);
          ...
          {step === 2 && (
            <div className="space-y-6">
              <label className="block text-sm font-medium text-stone-700 mb-2 flex items-center gap-2">
                <LayoutTemplate size={16} className="text-amber-500" /> Escolha
                um template para seu funil
              </label>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {t.popular && (
                          <span className="px-2 py-0.5 bg-amber-500/10 text-amber-700 rounded-full text-xs font-medium">
                            Mais usado
                          </span>
                        )}
                      </div>
                    ...
                    <div
                      className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${templateId === t.id ? "border-amber-500" : "border-stone-300"}`}
                    >
                      {templateId === t.id && (
