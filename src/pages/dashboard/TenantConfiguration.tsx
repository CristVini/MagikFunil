...
export function TenantConfiguration() {
  const { user } = useAuth();
  const [form, setForm] = useState<{ name: string; slug: string; email: string; whatsapp: string } | null>(null);
  ...
  useEffect(() => {
    // Carrega o tenant real (dono = auth.uid()). get_tenant_overview traz name/slug/whatsapp.
    supabase.rpc("get_tenant_overview").then(({ data, error: err }: { data: any; error: any }) => {
      if (err || !data?.tenant) return;
      const t = data.tenant;
      setForm({
        name: t.name || "",
        slug: t.slug || "",
  ...
  const saveName = async () => {
    ...
    const { error } = await supabase.from("tenants").update({ name: form.name }).eq("id", tenantId);
    setSaving(false);
  ...
  const saveWhatsapp = async () => {
    if (!form || !tenantId) return;
    setSaving(true); setSaved(false);
    const { error } = await supabase.from("tenants").update({ whatsapp: form.whatsapp }).eq("id", tenantId);
    setSaving(false);
    if (!error) { setSaved(true); setTimeout(() => setSaved(false), 2000); }
  };
    ...
    return (
      <div className="flex items-center justify-center py-32" style={{ fontFamily: "var(--font-sans)" }}>
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-stone-500">Carregando configurações...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={saveName} className="shrink-0 px-4 py-3 bg-stone-950 text-stone-50 rounded-xl font-medium hover:bg-stone-800 transition-colors flex items-center gap-2">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <input type="text" value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "") })} className={inputCls} disabled />
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={saveEmail} className="shrink-0 px-4 py-3 bg-stone-950 text-stone-50 rounded-xl font-medium hover:bg-stone-800 transition-colors flex items-center gap-2">
                {saving ? <LoaderIcon size={16} className="animate-spin" /> : <Save size={16} />} Salvar
              </button>
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={saveWhatsapp} className="shrink-0 px-4 py-3 bg-stone-950 text-stone-50 rounded-xl font-medium hover:bg-stone-800 transition-colors flex items-center gap-2">
                {saving ? <LoaderIcon size={16} className="animate-spin" /> : <Save size={16} />} Salvar
              </button>
            </div>
