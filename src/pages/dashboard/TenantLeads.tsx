...
export function TenantLeads() {
  ...
  const profilesAvailable = Array.from(
    new Set(leads.map((l) => l.profileLabel)),
  );
  ...
  const filtered = leads.filter((l) => {
    ...
    if (profileFilter !== "all" && l.profileLabel !== profileFilter)
      return false;
    if (search) {
      const s = search.toLowerCase();
      return (
        l.name.toLowerCase().includes(s) ||
        l.product_clicked.toLowerCase().includes(s)
      );
    }
    return true;
  });
  ...
  const updateStatus = async (id: string, status: any) => {
    // Otimista: atualiza a interface; depois persiste no banco
      ...
      {loading && (
        <div className="flex items-center justify-center py-24">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-stone-500">Carregando leads...</p>
          </div>
        </div>
      )}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-stone-500 mt-1">
                Quem chegou ao resultado do seu funil
              </p>
            </div>
            <button
              onClick={exportCSV}
                ...
                <tbody className="divide-y divide-stone-200">
                  {filtered.map((l) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 bg-stone-100 text-stone-700 rounded-full text-xs font-medium">
                          {l.profileLabel}
                        </span>
                      </td>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
              <div className="bg-white rounded-2xl max-w-lg w-full max-h-[80vh] overflow-y-auto p-6 animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3
                      className="text-xl font-display font-bold text-stone-950"
                      style={{ fontFamily: "var(--font-display)" }}
                    >
                      {selectedLead.name}
                    </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
                  <div className="bg-stone-50 rounded-xl p-3">
                    ...
                    <p className="font-medium text-stone-950">
                      {selectedLead.profileLabel}
                    </p>
                  </div>
                  <div className="bg-stone-50 rounded-xl p-3">
                    <p className="text-xs text-stone-500">Status</p>
                    <span
                      className={`inline-block px-2 py-0.5 mt-0.5 rounded-full text-xs font-medium ${STATUS_CONFIG[selectedLead.status].color}`}
                    >
                ...
                <p className="text-sm font-medium text-stone-700 mb-2">
                  Respostas do quiz (JSON)
                </p>
                <pre className="bg-stone-950 text-stone-100 rounded-xl p-4 text-xs overflow-x-auto whitespace-pre-wrap">
                  {JSON.stringify(selectedLead.answers, null, 2)}
                </pre>

                <button
                  onClick={() => setSelectedLead(null)}
                  className="mt-5 w-full py-2.5 bg-stone-950 text-stone-50 rounded-xl font-medium hover:bg-stone-800 transition-colors"
                >
                  Fechar
                </button>
              </div>
