...
function getKitOverride(id: string): {
  kit_name?: string;
  support_text?: string;
  price_cents?: number;
...
export function Result() {
  ...
  useEffect(() => {
    fetchQuiz(slug!).then(() => {
      ...
      setResult(r);
      // O tenant dono do funil já vem no get_funnel (com tema e whatsapp p/ CTA).
  ...
  if (!result) {
    return (
      <div
        className="min-h-screen flex items-center justify-center bg-[var(--theme-dark-background)]"
  ...
  const activeNotes = ((winner?.notes || []) as string[]).map(
    (note: string) => ({
      name: note,
      benefit: "Bem-estar",
  ...
  const handleIngredientClick = (note: any) => {
    ...
    useQuiz
      .getState()
      .trackEvent("product_click", { profile_id: winner?.id, note: note.name });
  };
  ...
  const handleItemClick = (prod: any) => {
    ...
    if (prod.redirect_url) {
      ...
      handleIngredientClick({
        name: prod.name,
        benefit: "Cuidado recomendado",
        description: prod.description || "Recomendado para o seu momento.",
        scientific_basis: (prod.key_actives || []).join(" + "),
  ...
  const handleCtaClick = () => {
    ...
    useQuiz
      .getState()
      .trackEvent("cta_click", { type: ctaAction.type, profile: winner?.id });
  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-2">
            <div
              ...
              style={{
                ...
                color: "var(--theme-dark-on-primary)",
              }}
            >
              <Sparkles size={20} />
            </div>
            <span
              className="font-display font-bold text-xl text-[var(--theme-dark-text)]"
      ...
      <main className="relative z-10 px-4 sm:px-6 py-6 mb-6">
        <div
          className="w-full max-w-7xl mx-auto border rounded-[40px] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.6)] animate-in fade-in zoom-in-98 duration-1000 backdrop-blur-2xl"
          style={{
          ...
          <div className="p-8 md:p-14 flex flex-col space-y-12">
                    ...
                    {runnerUp && runnerUp.score > 0 && (
                      <div className="space-y-2 pt-2">
                        ...
                        >
                          ...
                          ({runnerUp.archetype}). Considere combinar os dois
                          cuidados para uma experiência completa.
                        </p>
                      </div>
                    )}
                  </div>
              ...
              >
                ...
                <div className="flex flex-col gap-3">
                  {activeNotes.map((note: any, i: number) => (
                    ...
                    >
                      <div
                        className="w-10 h-10 rounded-full border flex items-center justify-center text-xs font-bold transition-colors group-hover:text-amber-500"
                        style={{
                          borderColor: "var(--theme-dark-border)",
                      ...
                      <span
                        className="text-xl md:text-2xl font-serif group-hover:translate-x-1 transition-all duration-500"
                        style={{
                          color: "var(--theme-dark-text)",
                          fontFamily: "var(--font-display)",
                        }}
                      ...
                      <span
                        className="ml-auto text-[10px] uppercase tracking-widest transition-colors group-hover:text-amber-500"
                        style={{ color: "var(--theme-dark-border)" }}
                      >
                        {note.benefit}
                      </span>
                      <Info
                        size={14}
                        className="transition-colors group-hover:text-amber-500/50"
                        style={{ color: "var(--theme-dark-border)" }}
                      />
                    </button>
                  ))}
              ...
              <div className="space-y-7">
                <div className="space-y-1">
                  ...
                  <p
                    ...
                    style={{ color: "var(--theme-dark-text-muted)" }}
                  >
                    Este é o passo a passo pensado para o seu momento. O kit é a
                    forma mais completa (e com melhor custo) de começar.
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-5">
                  {recommendedProducts.map((prod, i) =>
                    prod.show_promo && prod.promo_price_cents != null ? (
                      ...
                      <div
                        key={prod.id}
                        className="relative rounded-3xl p-6 overflow-hidden border-2 animate-in fade-in zoom-in-95 duration-500"
                        style={{
                        ...
                        <div className="relative flex flex-col gap-4">
                          ...
                          <div className="flex items-center gap-3">
                            <span
                              ...
                              style={{
                                ...
                                color: "var(--theme-dark-on-primary)",
                                boxShadow:
                                  "0 4px 16px -4px var(--theme-primary)",
                              }}
                            >
                          ...
                          <div className="flex items-end justify-between gap-4 flex-wrap">
                            <div className="min-w-0">
                              ...
                              >
                                {getKitOverride(prod.id).support_text ||
                                  prod.support_text ||
                                  "Protocolo de 30 dias · os produtos não precisam de receita"}
                              </p>
                            </div>
                            <div className="text-right shrink-0">
                              <p
                                className="text-[10px] font-bold uppercase tracking-widest flex items-center justify-end gap-1.5"
                                style={{
                                  color: "var(--theme-dark-text-muted)",
                                }}
                              >
                              ...
                              <div className="flex items-end justify-end gap-2">
                                {(() => {
                                  ...
                                  const normal =
                                    o.price_cents ?? prod.price_cents;
                                  if (normal && promo && normal > promo) {
                                    // Preço normal riscado + promocional grande
                                    return (
                                      <>
                                        <span
                                          className="text-2xl font-semibold line-through opacity-70"
                                          style={{
                                            color:
                                              "var(--theme-dark-text-muted)",
                                          }}
                                        >
                                          {brl(normal)}
                                        </span>
                          ...
                          <button
                            ...
                            style={{
                              backgroundColor: "var(--theme-primary)",
                              color: "var(--theme-dark-on-primary)",
                              boxShadow: "0 8px 30px -8px var(--theme-primary)",
                            }}
                          >
                            Peça este kit hoje
                            <ArrowRight size={18} />
                      ...
                      <div key={prod.id} className="flex gap-5 group/step">
                        <div
                          className="shrink-0 w-12 h-12 rounded-2xl border flex items-center justify-center transition-all duration-500"
                          style={{
                            borderColor: "var(--theme-dark-border)",
                            backgroundColor: "var(--theme-dark-background)",
                          }}
                        >
                          {getIcon(i)}
                        </div>
                  ...
                  {recommendedProducts.length === 0 && (
                    <p
                      ...
                      style={{ color: "var(--theme-dark-text-muted)" }}
                    >
                      Carregando protocolo recomendado...
                    </p>
                  )}
                </div>
                ...
                >
                  <span
                    ...
                    style={{ color: "var(--theme-dark-text-muted)" }}
                  >
                    Outros cuidados que combinam
                  </span>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {ranking.slice(1, 5).map((p) => (
                      <span
                        key={p.id}
                        className="px-3 py-1.5 border rounded-full text-[10px] font-medium"
                        style={{
                          backgroundColor: "var(--theme-dark-background)",
                          borderColor: "var(--theme-dark-border)",
                          color: "var(--theme-dark-text-muted)",
            ...
            >
              <div className="space-y-1 text-center md:text-left">
                ...
                <p
                  className="text-xl font-light max-w-md"
                  style={{ color: "var(--theme-dark-text)" }}
                >
                  Fale com nosso especialista para garantir seu cuidado
                  personalizado.
                </p>
              </div>
      ...
      {showScienceModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-xl p-4 md:p-8 animate-in fade-in duration-300">
          <div
            className="relative w-full max-w-2xl max-h-[90vh] border overflow-y-auto p-8 md:p-12 animate-in zoom-in-95 duration-300"
            style={{
              backgroundColor: "var(--theme-dark-surface)",
              borderColor: "var(--theme-dark-border)",
              borderRadius: "40px",
          ...
          >
            ...
            <h3
              className="text-3xl font-serif leading-tight mt-2 mb-6"
              style={{
                color: "var(--theme-dark-text)",
                fontFamily: "var(--font-display)",
              }}
